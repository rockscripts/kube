...
################################################################################################
            ...
            invoice_origin_document_journal_code = self.get_origin_document_journal_code(self.reversed_entry_id)
            ...
            sunat_data = {
                                ...
                                "documentoOrigen":self.reversed_entry_id.name,
...
################################################################################################
            ...
            invoice_origin_document_journal_code = self.get_origin_document_journal_code(self.debit_origin_id)
            sunat_origin_document_type = self.get_sunat_document_type(invoice_origin_document_journal_code)
            ...
            sunat_data = {
                                ...
                                "notaDescripcion":self.name,
                                "notaDiscrepanciaCode":self.discrepance_code,
                                "documentoOrigen":self.debit_origin_id.name,
                                "documentoOrigenTipo": sunat_origin_document_type, #01 factura, 03 boleta, 12 tiket de venta
                                "fechaEmision":str(invoice.invoice_date).replace("/","-",3),
                                "fechaVencimiento":str(invoice.invoice_date_due).replace("/","-",3),
                                "horaEmision":currentTime,
            ...
            qr_code_text = str(self.name) + str("|") + str(self.invoice_date)
            qr.add_data(qr_code_text)
            qr.make(fit=True)
            img = qr.make_image()
            temp = BytesIO()
    ...
    def can_create_notes(self):
        query = "select id, name as invoice_sequence_number_next from account_move where invoice_origin = '"+str(self.id)+"' and discrepance_code in ('01','02')"
        ...
        try:
            ...
            sunat_data =   {
                                ...
                                "reference":{
                                                ...
                                                "serie": str(secuencia_serie),
                                                "numero":str(secuencia_consecutivo),
                                                "fecha_emision":str(self.invoice_date).replace("/","-",3),
                                ...
                                "emisor":{
                                            ...
                                            "ubigeo":(str(self.company_id.ubigeo) if(self.company_id.ubigeo!=None) else str(self.company_id.zip))
                                        },
                                "sunat_sol": {
    ...
    def get_origin_document_journal_code(self, reversed_entry_id):
        query = "select account_move.journal_id, account_journal.code from account_move left join account_journal on account_move.journal_id = account_journal.id where account_move.id = '"+str(reversed_entry_id.id)+"'"
        request.cr.execute(query)
    ...
    def get_sunat_product_code_classification(self, item_id):
        try:
            ...
            return str("no requerido en factura nacional")
    ...
    def get_payment_terms(self):
        days = int(0)        
        response = {'debit':None, 'credit':[], 'type':str('debit')}
        if(self.invoice_payment_term_id):
            pass
        else:
            self.invoice_payment_term_id = request.env['account.payment.term'].sudo().search([], limit=1, order='id asc')
        _logger.warning('get_payment_terms days')            
        if(self.invoice_payment_term_id.line_ids):
            index = int(1)                
            for _line in self.invoice_payment_term_id.line_ids:
                _logger.warning(_line.read())
                if(index == 1 ):
                    days = int(_line.days)
                index += int(1)
        
        _logger.warning(int(days))

        if(int(days)==int(0)):
            response['debit'] = {'ID': 'FormaPago', 'PaymentMeansID':'Contado'}
        else:
            response['credit'].append({
                                        'ID': 'FormaPago', 
                                        'PaymentMeansID': 'Credito', 
                                        'Amount': float(0),
                                        'PaymentDueDate': None
                                        })
            response['type'] = str('credit')
    ...
    def edocs_submit_invoices(self, **kw):
        ...
        query = "select nextcall from ir_cron where cron_name = '"+str("SUNAT_Comprobantes_Electronicos")+"'"
        request.cr.execute(query)
        ...
        moves = request.env['account.move'].sudo().search([('invoice_date','<=',str(invoice_date_limit))])
        
        for invoice_unsubmited in moves:
            query = "select res_partner.vat, res_company.api_mode, res_company.sol_ruc, res_company.sol_username, res_company.sol_password, res_company.certs from res_company left join res_partner on res_partner.company_id = res_company.id where res_company.id = "+str(invoice_unsubmited.company_id.id)+" and res_partner.is_company = TRUE and res_company.partner_id = res_partner.id"
            request.cr.execute(query)
            company_fields = request.cr.dictfetchone()

            if(invoice_unsubmited.invoice_sequence_number_next):                  
                sunat_data = {
                                "secuencia_consecutivo":str(invoice_unsubmited.invoice_sequence_number_next_prefix) + str(invoice_unsubmited.invoice_sequence_number_next),
                                "numero":str(invoice_unsubmited.invoice_sequence_number_next),
                                "serie":str(invoice_unsubmited.invoice_sequence_number_next_prefix).replace('-',''),
                                "sunat_sol": {
                ...
                if("F0" in str(invoice_unsubmited.invoice_sequence_number_next_prefix)):
                    SunatService.fileName = str(company_fields["vat"])+"-01-"+str(invoice_unsubmited.invoice_sequence_number_next_prefix)+""+str(invoice_unsubmited.invoice_sequence_number_next_prefix)
                    SunatService.documentType = str("01")
                if("BF" in str(invoice_unsubmited.invoice_sequence_number_next_prefix)):
                    SunatService.fileName = str(company_fields["vat"])+"-03-"+str(invoice_unsubmited.invoice_sequence_number_next_prefix)+""+str(invoice_unsubmited.invoice_sequence_number_next_prefix)
                    SunatService.documentType = str("03")
                if("FD" in str(invoice_unsubmited.invoice_sequence_number_next_prefix)):
                    SunatService.fileName = str(company_fields["vat"])+"-08-"+str(invoice_unsubmited.invoice_sequence_number_next_prefix)+""+str(invoice_unsubmited.invoice_sequence_number_next_prefix)
                    SunatService.documentType = str("08")
                if("FC" in str(invoice_unsubmited.invoice_sequence_number_next_prefix)):
                    SunatService.fileName = str(company_fields["vat"])+"-07-"+str(invoice_unsubmited.invoice_sequence_number_next_prefix)+""+str(invoice_unsubmited.invoice_sequence_number_next_prefix)
                ...
                _logger.warning('sunatResponse')
                _logger.warning(sunatResponse)

                if(sunatResponse["status"] == "OK"):
                    response_document_filename = str("R_")+"-01-"+str(invoice_unsubmited.invoice_sequence_number_next_prefix)+str(invoice_unsubmited.invoice_sequence_number_next_prefix)+str(".XML")
                    api_message = "ESTADO: "+str(sunatResponse["status"])+"\n"+"REFERENCIA: "+str(sunatResponse["body"]["referencia"]).replace("'",'"')+"\n"+" "+str(sunatResponse["body"]["description"]).replace("'",'"')
                    query = "update account_move set sunat_request_status = 'OK', api_message = '"+str(api_message)+"', response_document_filename = '"+str(response_document_filename)+"', sunat_request_type = 'Automatizada' where id = "+str(invoice_unsubmited.id)
                    request.cr.execute(query)
                else:
                    api_message = "ESTADO: "+str(sunatResponse["status"])+"\n"+" "+str(sunatResponse["body"]).replace("'",'"')+"\n"+"CÓDIGO ERROR: "+str(sunatResponse["code"]).replace("'",'"')
                    query = "update account_move set sunat_request_status = 'FAIL', api_message = '"+str(api_message)+"', sunat_request_type = 'Automatizada' where id = "+str(invoice_unsubmited.id)
                    request.cr.execute(query)