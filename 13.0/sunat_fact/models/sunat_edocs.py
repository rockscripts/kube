...
class SunatEdocs(models.Model):
    ...
    invoice_sequence_number_next = fields.Char(string='Documento', readonly=True)
    invoice_date = fields.Date(string='Creado en', readonly=True)
    ...
    def init(self):
        ...
        self._cr.execute(
                         """
                            ...
                            ( 
                              SELECT invoice_sequence_number_next as invoice_sequence_number_next, 
                              ...
                              WHERE account_invoice.state = 'open' and account_invoice.signed_document!='' and account_invoice.invoice_sequence_number_next!=''
    ...
    def edocs_submit_invoices(self, **kw):
        ...
        query = "select id, invoice_sequence_number_next, company_id, unsigned_document, signed_document, response_document from account_invoice where invoice_date <= '"+str(invoice_date_limit)+"' and (sunat_request_status = '"+str("FAIL")+"' or sunat_request_status = '"+str("No Emitido")+"' or sunat_request_status = '"+str("not_requested")+"')"
        request.cr.execute(query)
        ...
        for invoice_unsubmited in invoices_unsubmited:
            ...
            if(invoice_unsubmited["invoice_sequence_number_next"]):
                ...
                serieParts = str(invoice_unsubmited["invoice_sequence_number_next"]).split("-")  
