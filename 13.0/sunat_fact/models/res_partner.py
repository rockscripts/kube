...
class res_partner(models.Model):
    ...
    sunat_tipo_documento = fields.Selection([('6','RUC'),('1','DNI'),('4','Carnet extrangeria'),('7','Pasaporte'),('A','CED. DIPLOMATICA DE IDENTIDAD'),('B','DOC.IDENT.PAIS.RESIDENCIA-NO.D'),('C','Tax Identification - TIN – Doc Trib PP.NN'),('D','Identification - IN – Doc Trib PP. JJ')], string='Tipo Documento', default='6')
    street_name = fields.Char(string="Domicilio", default="")
    document_number = fields.Char(default="-")
    _columns = {"sunat_tipo_documento":fields.Selection([('0','DOC.TRIB.NO.DOM.SIN.RUC'),('1','DNI'),('4','CARNET DE EXTRANJERIA'),('6','REG. UNICO DE CONTRIBUYENTES'),('7','PASAPORTE'),('A','CED. DIPLOMATICA DE IDENTIDAD'),('B','DOC.IDENT.PAIS.RESIDENCIA-NO.D'),('C','Tax Identification - TIN – Doc Trib PP.NN'),('D','Identification - IN – Doc Trib PP. JJ')], string='Tipo Documento', default='6')}

    @api.model
    def search_partner_default(self, partner):
        _partner = request.env['res.partner'].sudo().search_read([('id','=',int(partner))])
        if(_partner):
            return _partner
        else:
            return None
    ...
    def create_from_ui(self, partner):
        if(partner['sunat_tipo_documento'] == '6'):
        ...
        if self.sunat_tipo_documento and self.sunat_tipo_documento == '1':
           #Valida DNI
            ...
            if self.vat and len(self.vat) != 11:
                ...
                dist_id = ditrict_obj.search([('code', '=', str(d['ubigeo']))], limit=1)

                _logger.warning('ubigeo')
                _logger.warning(d['ubigeo'])
                _logger.warning(dist_id)
                ...
                social_reason_name = str(d['nombre'])
                company_name = str(d['nombre_comercial'])
                _logger.warning(d)

                self.name = str(social_reason_name).upper()
                self.company_name = str(company_name).replace("\r\n            \r\n            \t","").upper()
                self.street = str(d['domicilio_fiscal']).upper()
                self.street_name = str(d['domicilio_fiscal']).upper()
    ...
    def on_street_name(self): 
        self.street = self.street_name
    
    @api.onchange('zip')
    def on_zip(self): 
        self.ubigeo = self.zip
