...
class stock_picking(models.Model):
            ...
            for pick in self:
                if pick.id:
                    ...
                    for stock_pick in stocks_picking:
                        ...
                        if(not carrier):
                            notification = {
                                ...
                                'params': {
                                    'title': ('* Requerido'),
                                    'message': str("El transportista debe ser seleccionado"),
                                    'type':'danger',
                                    'sticky': True,
                                },
                            }
                            
                            _logger.warning("action_done")
                            _logger.warning("El transportista debe ser seleccionado")
                    ...
                    if(data['transportista']['nro']==False):
                        ...
                        error_message = 'Debes establecer un transportista'

                    if(data['receptor']['nro']==False):
                        #raise Warning('Debes establecer un receptor')
                        error_message = 'Debes establecer un receptor'
                    ...
                    if(bool(Vcompany_address["errors"])==True):
                        ...
                        error_message = str("Compañia emisora")+str(Vcompany_address["msn"])
                    ...
                    if(bool(Vcompany_certificate["errors"])==True):
                        ...
                        error_message = str("Compañia emisora")+str(Vcompany_certificate["msn"])
                    ...
                    if(bool(Vpartner_address["errors"])==True):
                        ...
                        error_message = str("Cliente / Socio")+str(Vpartner_address["msn"])
                    ...
                    qr_code_text = str(data["emisor"]["nro"]) + str("|") + str(data["emisor"]["tipo"]) + str("|") + str(data["serie"]) + str("|") + str(data["numero"])+ str("|") + str(data["fechaEmision"]) + str("|") + str(data["receptor"]["tipo"]) + str("|") + str(data["receptor"]["nro"])
    ...
    def validate_address(self, res_model):
        ...
        _logger.warning("res_model.street_name")
        _logger.warning(res_model.street_name)

        if((res_model.street_name==False or res_model.street_name=='' or not res_model.street_name)):
            response["errors"] = True
            errors_msn = errors_msn + str(" \n - Establecer dirección de envío / ubigeo")
