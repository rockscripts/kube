...
class SunatEfact(http.Controller):
        ...
        try:
            if(doc_num!=""):
                ...
                if len(response_service['data'])>0:
                    if(int(doc_type)==6):
                        query = "select id, name, code from res_country_state where length(code) = 6 and country_id = "+str(173)+" and  code = '"+ str(response_service['data']['ubigeo'])+"'"
                        request.cr.execute(query)
                        district = request.cr.dictfetchone()
                        ...
                        if(district):
                            ubigeo = response_service['data']['ubigeo']
                            district_id = request.env['res.country.state'].sudo().browse(district['id'])
                            province_id = district_id.province_id.name
                            state_id = district_id.state_id.name
                            district_id = district_id.code
                        ...
                        social_reason_name = str(response_service["data"]['nombre']).upper()
                        nombre_comercial = str(response_service["data"]['nombre_comercial']).replace("\r\n            \r\n            \t","")

                        _logger.warning(response_service)
                        response = {
                                        'status' :  "OK",
                                        'nmro':doc_num,
                                        'address' : str(response_service["data"]['domicilio_fiscal']).upper(),
                                        'name' : social_reason_name,
                                        'tipo_contribuyente' : response_service["data"]['tipo_contribuyente'],
                                        'nombre_comercial' : str(nombre_comercial).upper(),
    ...
    def can_create_notes(self, **kw):
        invoice_invoice_sequence_number_next = kw.get('invoice_invoice_sequence_number_next')
        #query = "select id, invoice_sequence_number_next from account_move where ref = '"+str(invoice_invoice_sequence_number_next)+"' and name li and discrepance_code in ('01','02')"
        query = "select id, name from account_move where ref ILIKE '%"+str(invoice_invoice_sequence_number_next)+"%' and name ILIKE '%FC%' and discrepance_code in ('01','02')"
        request.cr.execute(query)
        refund_invoice = request.cr.dictfetchone()
        if(refund_invoice):
            refund_invoice["name"] = refund_invoice["name"]
            refund_invoice["found"] = True
        else: 
            refund_invoice = {}
            refund_invoice["found"] = False
        refund_invoice["query"] = query
    ...
    def get_invoice_ordered(self, **kw):
        ...
        response = {}
        query = "select account_move from pos_order where pos_reference = '"+str(orderID)+"'"
        ...
        try:
            ...
            invoice_id = pos_sale['account_move']
            query = "select id, name, qr_image, sunat_total_letters, journal_id from account_move where id = "+str(invoice_id)
            request.cr.execute(query)    
            account_invoice = request.cr.dictfetchone()
            if(account_invoice):
                account_invoice['qr_image'] = str('data:image/png;base64,') + str(account_invoice['qr_image'])
            ...
            exc_traceback = sys.exc_info()
            _logger.warning(str(repr(e))+" ON LINE "+format(sys.exc_info()[-1].tb_lineno))
            raise Warning(str(repr(e))+" ON LINE "+format(sys.exc_info()[-1].tb_lineno))
    ...
    def get_invoice_ticket_journal(self, **kw):
        ...
        query = "select id, name, code from account_journal where code in ('INV','FAC','BOL','NVFAC')"
        request.cr.execute(query)    
    ...
    def save_representants(self, **kw):
        ...
        doc_type = kw.get('doc_type')
        doc_invoice_sequence_number_next = kw.get('doc_invoice_sequence_number_next')
        name = kw.get('name')
        ...
        if(int(id_representant)==0):
            ...
            params ["doc_invoice_sequence_number_next"] = doc_invoice_sequence_number_next
            params ["doc_type"] = doc_type
            representant = self.get_representant(params)
            if(representant):
                return False
            else:
                query = "insert into res_representants (id_company, doc_type, doc_invoice_sequence_number_next, name, position, address, date_added) values ('"+str(doc_type)+"', '"+str(doc_type)+"', '"+str(doc_invoice_sequence_number_next)+"', '"+str(name)+"', '"+str(position)+"', '"+str(address)+"', '"+str(date_added)+"')"
        else:
            query = "update res_representants set doc_type='"+str(doc_type)+"', doc_invoice_sequence_number_next='"+str(doc_invoice_sequence_number_next)+"', name='"+str(name)+"', position='"+str(position)+"', address='"+str(address)+"' where id='"+str(id_representant)+"'"
    ...
    def get_representant(self,data):
        if (data["search_type"]=="check_exist"):
            query = "select * from res_representants where id_company='"+str(data["id_company"])+"' and doc_invoice_sequence_number_next = '"+str(data["doc_invoice_sequence_number_next"])+"' and doc_type = '"+str(data["doc_type"])+"'"
        else:
    ...
    def edocs_submit_invoice(self, **kw):
        ...
        query = "select invoice_sequence_number_next, company_id, unsigned_document, signed_document, response_document from account_invoice where id = "+str(invoice_id)
        request.cr.execute(query)
        ...
        serieParts = str(invoice_fields["invoice_sequence_number_next"]).split("-")             
        serieConsecutivoString = serieParts[0]
        serieConsecutivo = serieParts[1]
    ...
    def update_inv(self, **kw):
        query = "select id, invoice_sequence_number_next, origin, move_name, reference from account_invoice where invoice_sequence_number_next != ''"
        request.cr.execute(query)
        invoices = request.cr.dictfetchall()
        strQ = str("")
        try:
            for invoice in invoices:
                move_name = str(invoice['move_name']).replace(str(invoice['invoice_sequence_number_next']),str(invoice['origin']))
                refer = str(invoice['reference']).replace(str(invoice['invoice_sequence_number_next']),str(invoice['origin']))
                query = "update account_invoice set invoice_sequence_number_next = '"+str(invoice['origin'])+"', move_name='"+str(move_name)+"' , reference='"+str(refer)+"' where id = "+str(invoice['id'])+str(";")              
                strQ = str(strQ) + str('\n') + str(query)
                #request.cr.execute(query)
