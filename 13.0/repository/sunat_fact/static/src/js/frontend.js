odoo.define('module.SunatEinvoicing', function(require) {
    ...
    $(document).ready(function() {
            ...
            {
                $('div.div_city').find('label').text('Ciudad')
                var province_id = $("select[name='province_id']").val()
                if(province_id==null)
                {
                    var country_id = $("select[name='country_id']").val();
                    var x_state_id = ($("input[id='x_state_id']").val()!=null) ? $("input[id='x_state_id']").val() : 0;
                    var x_province_id = ($("input[id='x_province_id']").val()!=null) ? $("input[id='x_province_id']").val() : 0;
                    var x_district_id = ($("input[id='x_district_id']").val()!=null) ? $("input[id='x_district_id']").val() : 0;
                    populate_location(parseInt(country_id), parseInt(x_state_id), parseInt(x_province_id), parseInt(x_district_id));                    
        ...
        $(document).on("blur", "input[name='ubigeo']", function() {
            var ubigeo = $("input[name='ubigeo']").val();
            var zip = $("input[name='zip']").val();
            if (toString(ubigeo) != "") {
                var zip = $("input[name='zip']").val(ubigeo);
            }
        });
        // }, mainIntervalTime);
        ...
        $(document).on("change", "select[name='state_id']", function () {
            ...
            if(district_id==0)
            {
                var x_district_id = $("input[id='x_district_id']").val();
                if(parseInt(x_district_id)>0)
                {
                    district_id = x_district_id;
                    $("input[id='x_district_id']").val(0);
                }
            }
    ...
    function setClientDetails() {
        ...
        if (documentType == "6" || documentType == "1") {
            var documentinvoice_sequence_number_next = $("input[name=vat]").val();
            if (documentinvoice_sequence_number_next != "") {
        ...
        $.ajax({
            ...
            success: function (response) {
               if (response.result) {
                   ...
                   $("input[name='ubigeo']").val(district.code);
                   $("input[name='zip']").val(district.code);
               }
            }
        });
    }
