    ...
    var mainInterval = setInterval(function() {
        ...
        $(document).on("click", ".btn-representante-save", function() {
            ...
            if ($(".representantes-form-container").length > 0) {
                ...
                var doc_invoice_sequence_number_next = $("#doc_invoice_sequence_number_next").val();
                var name = $("#name").val();
                var position = $("#position").val();
                var address = $("#address").val();
                if (doc_invoice_sequence_number_next == "" || name == "" || position == "" || address == "") {
                    swal({
                ...
                var data = { "params": { "id_company": id_company, "id_representant": id_representant, "doc_type": doc_type, "doc_invoice_sequence_number_next": doc_invoice_sequence_number_next, "name": name, "position": position, "address": address } }
                $.ajax({
                    type: "POST",
        ...
        $(document).on("click", ".btn-representante-edit", function() {
            ...
            data["doc_invoice_sequence_number_next"] = rowContainer.find(".td-doc_invoice_sequence_number_next").text();
            ...
            $("#doc_invoice_sequence_number_next").val(data["doc_invoice_sequence_number_next"]);
            $("#name").val(data["name"]);
...
function loadRepresentantForm() {
    ...
    form += '<label for="doc_invoice_sequence_number_next">Número de Documento</label>';
    form += '<div><input id="doc_invoice_sequence_number_next" type="text"/></div>';
    form += '</div>';
...
function loadGeneralInformationByDocument() {
    if($(".company-general-information").length>0)
    {
    var documentinvoice_sequence_number_next = $("span[name=vat]").val();
    ...
    $(".o_field_char").each(function(index, item) {
        if (item.getAttribute("name") == "vat") {
            documentinvoice_sequence_number_next = $(item).text();
        }
    ...
    $(".o_field_widget").each(function(index, item) {
        if (item.getAttribute("name") == "sunat_tipo_documento") {
            ...
            if (documentType == "DOC.IDENT.PAIS.RESIDENCIA-NO.D")
                documentType = "B"
            if (documentType == "Tax Identification invoice_sequence_number_next - TIN – Doc Trib PP.NN")
                documentType = "C"
            if (documentType == "Identification invoice_sequence_number_next - IN – Doc Trib PP. JJ")
                documentType = "D"
                //alert("2 " + documentType)
        }
    });  

    if (parseInt(documentinvoice_sequence_number_next) < 0) {
        var documentinvoice_sequence_number_next = $(".company_vat").val();
    }
    ...
    else
    {
        ...
        if (documentType == "Identification invoice_sequence_number_next - IN – Doc Trib PP. JJ")
            documentType = "D"
        documentinvoice_sequence_number_next  = $("input[name='vat']").val();
    }
    if (documentinvoice_sequence_number_next == null || documentinvoice_sequence_number_next == '')
    {
        return false;
    }
    if (parseInt(documentinvoice_sequence_number_next) > 0) {
