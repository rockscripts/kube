    ...
    ******************************************************** */
                        ...
                        success: function (response) {
                            if (response.result.journals != null && response.result.journals != '') {
                                ...
                                journals.forEach(function (journal, index) {
                                    if (journal.id > 0) {
                                        option += '<option value="' + journal.id + '">' + journal.name + '</option>';
                                        if (String(String(journal.name).toLocaleLowerCase()).includes('FACT') || String(String(journal.name).toLocaleLowerCase()).includes('INV')) {
                                            journal_factura_id = journal.id;
                                            document_id = journal.id;
                                        }
                                        if (String(String(journal.code).toLocaleLowerCase()).includes('BOL')) {
                                            journal_boleta_id = journal.id;
                                            document_id = journal.id;
                                        }
                                        if (String(String(journal.code)).includes('NVFAC')) {
    ...
    screens.ReceiptScreenWidget.include({
        get_receipt_render_env: function () {
            ...
            this.pos.last_receipt_render_env['einvoice_sequence_number_next'] = pos_edoc['invoice_sequence_number_next'];
            this.pos.last_receipt_render_env['total_letters'] = pos_edoc['total_letters'];
            this.pos.last_receipt_render_env['qr_image'] = window.location.origin + '/web/image?model=account.move&field=qr_image&id=' + pos_edoc['inv_id'];
            ...
            if (orderID != "") {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        result['invoice_sequence_number_next'] = response.result.invoice_sequence_number_next;
...
odoo.define("sunat_fact.efact_PosModels", function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            company_model.fields.push('street_name');
    ...
    models.PosModel = models.PosModel.extend({
        initialize: function (session, attributes) {
            ...
            partner_model.fields.push('street_name');
