...
class ClearDataModel(models.Model):
    ...
    model_clear_ids=fields.Many2many("ir.model",'clear_data_ir_model_rel','group_id','model_id',domain=[('model','not in',['clear.data.model'])],ondelete="set default")
    ...
    def _do_clear_ir_sequence(self):
        ...
        #清除不标准
        cr.execute("update ir_sequence set invoice_sequence_number_next_next=1")
        cr.execute("update ir_sequence_date_range set invoice_sequence_number_next_next=1")
        #清除标准
